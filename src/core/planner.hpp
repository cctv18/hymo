...
struct MountPlan {
    ...
    // Helper to check if a path is covered by an overlay operation
...
// Updates the kernel-side HymoFS path mappings
void update_hymofs_mappings(
    const Config& config,
    const std::vector<Module>& modules,
    const fs::path& storage_root,
    const MountPlan& plan // Added plan to check for exclusions
);
